...
public class Camera : AbstractGameObject
{
    ...
    public Camera(short cameraType = Perspective) : base(metadata: cameraType)
    {
        switch (cameraType)
        {
            case Perspective:
    ...
    public float FOV { get; set; } = 1.8f;
    public float NearPlane { get; set; } = 10;
    public float FarPlane { get; set; } = 1000;

    internal Matrix4x4 ViewMatrix => Matrix4x4.CreateLookAt(Transform.Position,
        Vector3.Transform(Transform.Position, Transform.Rotation), Vector3.UnitY);
